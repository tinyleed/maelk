...
- Export a Cloudflare Worker entry that adapts the API-only Express app through `httpServerHandler` and consumes the typed `MAELK_SESSION_HYPERDRIVE.connectionString` binding for Worker session storage.
...
`DATABASE_URL`, Worker Hyperdrive connection strings, refresh tokens, cookie identifiers, OTPs, `MAELK_SESSION_ENCRYPTION_KEY`, database passwords, and hosted service credentials must never be exposed to the browser or committed. Node/Express uses `DATABASE_URL`; Worker production auth uses `MAELK_SESSION_HYPERDRIVE.connectionString` and fails closed with sanitized unavailable responses when the binding or other production auth settings are missing.
...
The Worker preview gate bundles without upload, starts only on loopback, and verifies same-origin Express API plus SPA/static-asset routing with credential-free fail-closed auth. It does not verify hosted Worker-to-Postgres connectivity; see `../../docs/cloudflare-workers-preview.md`.
